/**
 * @file dlms_aes.c
 * @brief AES-128 Implementation (FIPS 197)
 */
...
void dlms_aes128_set_key_enc(dlms_aes128_ctx_t *ctx, const uint8_t key[16]) {
    ...
    for (int i = 4; i < 4 * (DLMS_AES_ROUNDS + 1); i++) {
        uint8_t temp[4];
...
void dlms_aes128_set_key_dec(dlms_aes128_ctx_t *ctx, const uint8_t key[16]) {
    dlms_aes128_set_key_enc(ctx, key);
    /* For standard Equivalence InvMixColumns if using table-based decryption,
       or direct decryption by running rounds in reverse with inv Sbox & inv MixColumns */
}
...
void dlms_aes128_encrypt_block(const dlms_aes128_ctx_t *ctx, const uint8_t in[16], uint8_t out[16]) {
    ...
    for (int round = 1; round < DLMS_AES_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
    ...
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk + DLMS_AES_ROUNDS * 16);
...
void dlms_aes128_decrypt_block(const dlms_aes128_ctx_t *ctx, const uint8_t in[16], uint8_t out[16]) {
    ...
    for (int round = DLMS_AES_ROUNDS - 1; round > 0; round--) {
